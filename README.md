...
- **Security Center**: Enhanced firewall controls now support macOS and the dialog lists all listening ports with the owning process for quick audits. Listeners refresh automatically and you can filter them by port or name. Terminate one process or its entire tree with a single click or kill whole port ranges at once. Windows Defender toggling and automatic elevation remain built in so settings always apply successfully. When launched without admin rights, only the Security Center is re-spawned with elevation instead of restarting the entire app. A "+" button beside each toggle opens dedicated Firewall and Defender dialogs for deeper control. These dialogs now slide in and arrange themselves on whichever side of the main window has space—left, right, above, or below—and firewall/Defender statuses are color-coded for quick scanning.
- **Capability Governance**: The Security Center now includes a dedicated Tools/Plugins tab. Review every running worker, the capabilities it was granted, and the runtime insights reported by the supervisor (open ports, recent syscalls, and a summarized process tree). Pending capability prompts appear both here and in the Tools view entry point so you can triage them quickly.
- **Permission Prompts & Revocation**: Capability requests support "allow once", "allow 15 minutes", and "always" responses. Granted capabilities automatically downgrade after inactivity and can be downgraded or revoked at any time from the new tab, reusing the existing elevation flow when administrator approval is required.
