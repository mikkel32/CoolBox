...
spec = importlib.util.spec_from_file_location(
    ...
    pathlib.Path("src/coolbox/ui/views/dialogs/force_quit.py"),
)
...
sys.modules["coolbox.ui.views.dialogs.force_quit"] = force_quit_dialog
...
def test_kill_by_click_selects_and_kills_pid() -> None:
    ...
    with (
        ...
        patch("coolbox.ui.views.dialogs.force_quit.messagebox") as MB,
        patch("coolbox.ui.views.dialogs.force_quit.psutil.pid_exists", return_value=True),
        patch("coolbox.ui.views.dialogs.force_quit.psutil.Process") as Proc,
    ):
...
def test_kill_by_click_cancel_does_not_kill() -> None:
    ...
    with (
        patch.dict(os.environ, {"FORCE_QUIT_CLICK_SKIP_CONFIRM": "1"}),
        patch("coolbox.ui.views.dialogs.force_quit.messagebox"),
    ):
        dialog._configure_overlay()
...
def test_kill_by_click_cancel_before_start_skips_choose() -> None:
    ...
    with patch("coolbox.ui.views.dialogs.force_quit.messagebox"):
        dialog._configure_overlay()
        dialog._kill_by_click()
        dialog.cancel_kill_by_click()
        time.sleep(0.2)
...
def test_kill_by_click_skips_vanished_process() -> None:
    ...
    dialog.after = lambda delay, cb, *args: threading.Timer(delay / 1000.0, cb, args).start()
    with (
        patch("coolbox.ui.views.dialogs.force_quit.psutil.pid_exists", return_value=False),
        patch("coolbox.ui.views.dialogs.force_quit.messagebox") as MB,
        patch("builtins.print") as mock_print,
    ):
        ctx = dialog._OverlayContext(dialog, overlay)
...
def test_force_kill_reports_when_process_resists(capsys) -> None:
    with (
        patch("coolbox.ui.views.dialogs.force_quit.kill_process", return_value=False) as kp,
        patch(
            "coolbox.ui.views.dialogs.force_quit.kill_process_tree",
            return_value=False,
        ) as kpt,
        patch("coolbox.ui.views.dialogs.force_quit.psutil.pid_exists", return_value=True),
        patch("coolbox.ui.views.dialogs.force_quit.psutil.Process") as Proc,
