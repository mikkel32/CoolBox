...
import scripts.run_vm_dev as rvd
...
def test_main_list_backends(monkeypatch, capsys):
    ...
    monkeypatch.setattr(rvd.sys, "argv", ["run_vm_dev.py", "--list"])
    rvd.main()
...
def test_main_passes_port(monkeypatch):
    ...
    monkeypatch.setattr(rvd.sys, "argv", ["run_vm_dev.py", "--port", "9999"])
    rvd.main()
