...
class TestClickOverlay(unittest.TestCase):
    ...
    def test_query_reuses_cached_rect(self) -> None:
        ...
        overlay.state = OverlayState.HOOKED
        cached = WindowInfo(7, (0, 0, 10, 10), "cached")
        overlay._cached_info = cached

        with (
            patch.object(overlay.engine.tracker, "best_with_confidence") as best,
            patch.object(overlay, "_probe_point") as probe,
        ):
            info = overlay._query_window_at(5, 5)
    ...
    def test_velocity_weight_reduces_sample_influence(self) -> None:
        ...
        with (
            patch("coolbox.utils.analysis.scoring_engine.tuning.sample_weight", 1.0),
            patch("coolbox.utils.analysis.scoring_engine.tuning.history_weight", 1.0),
            patch("coolbox.utils.analysis.scoring_engine.tuning.velocity_scale", 1.0),
        ):
            choice = overlay._weighted_choice(samples)
    ...
    def test_stability_weight_influences_choice(self) -> None:
        ...
        with (
            patch("coolbox.utils.analysis.scoring_engine.tuning.sample_weight", 1.0),
            patch("coolbox.utils.analysis.scoring_engine.tuning.history_weight", 1.0),
            patch("coolbox.utils.analysis.scoring_engine.tuning.stability_weight", 2.0),
        ):
            choice = overlay._weighted_choice(samples)
    ...
    def test_stable_info_requires_threshold(self) -> None:
        ...
        with patch("coolbox.utils.analysis.scoring_engine.tuning.stability_threshold", 2):
    ...
    def test_center_weight_biases_selection(self) -> None:
        ...
        with (
            ...
            patch("coolbox.utils.analysis.scoring_engine.tuning.center_weight", 5.0),
        ):
            choice = overlay._weighted_choice(samples)
    ...
    def test_edge_penalty_discourages_borders(self) -> None:
        ...
        with (
            patch("coolbox.utils.analysis.scoring_engine.tuning.sample_weight", 1.0),
            patch("coolbox.utils.analysis.scoring_engine.tuning.history_weight", 0.0),
            patch("coolbox.utils.analysis.scoring_engine.tuning.edge_penalty", 0.9),
            patch("coolbox.utils.analysis.scoring_engine.tuning.edge_buffer", 2),
        ):
            choice = overlay._weighted_choice(samples)
    ...
    def test_velocity_scaled_stability(self) -> None:
        ...
        with (
            patch("coolbox.utils.analysis.scoring_engine.tuning.stability_threshold", 2),
            patch("coolbox.utils.analysis.scoring_engine.tuning.vel_stab_scale", 2.0),
        ):
            info = overlay._stable_info()
    ...
    def test_path_weight_favors_hovered_window(self) -> None:
        ...
        with (
            patch("coolbox.utils.analysis.scoring_engine.tuning.sample_weight", 1.0),
            patch("coolbox.utils.analysis.scoring_engine.tuning.history_weight", 0.0),
            patch("coolbox.utils.analysis.scoring_engine.tuning.path_weight", 3.0),
        ):
            choice = overlay._weighted_choice(samples)
    ...
    def test_heatmap_weight_biases_selection(self) -> None:
        ...
        with (
            patch("coolbox.utils.analysis.scoring_engine.tuning.sample_weight", 1.0),
            patch("coolbox.utils.analysis.scoring_engine.tuning.history_weight", 0.0),
            patch("coolbox.utils.analysis.scoring_engine.tuning.heatmap_weight", 2.0),
        ):
            choice = overlay._weighted_choice(samples)
    ...
    def test_streak_weight_amplifies_current_pid(self) -> None:
        ...
        with (
            patch("coolbox.utils.analysis.scoring_engine.tuning.sample_weight", 1.0),
            patch("coolbox.utils.analysis.scoring_engine.tuning.history_weight", 0.0),
            patch("coolbox.utils.analysis.scoring_engine.tuning.streak_weight", 2.0),
        ):
            choice = overlay._weighted_choice(samples)
    ...
    def test_recency_weight_biases_recent_pid(self) -> None:
        ...
        with patch("coolbox.utils.analysis.scoring_engine.tuning.recency_weight", 5.0):
            choice = overlay._weighted_choice(samples)
    ...
    def test_duration_weight_biases_longest_pid(self) -> None:
        ...
        with patch("coolbox.utils.analysis.scoring_engine.tuning.duration_weight", 3.0):
            choice = overlay._weighted_choice(samples)
    ...
    def test_tracker_ratio_used_when_query_fails(self) -> None:
        ...
        with (
            ...
            patch("coolbox.utils.analysis.scoring_engine.tuning.tracker_ratio", 2.0),
        ):
            overlay.close = lambda _e=None: None
            overlay._on_click()
    ...
    def test_confirm_weight_updates_pid(self) -> None:
        ...
        with (
            ...
            patch("coolbox.utils.analysis.scoring_engine.tuning.confirm_weight", 5.0),
        ):
            overlay.close = lambda _e=None: None
            overlay._on_click()
    ...
    def test_gaze_weight_biases_selection(self) -> None:
        ...
        with patch("coolbox.utils.analysis.scoring_engine.tuning.gaze_weight", 5.0):
            choice = overlay._weighted_choice(samples)
...
def test_apply_updates_uses_move_for_translation() -> None:
    ...
    from coolbox.ui.views import click_overlay
