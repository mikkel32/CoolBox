...
import coolbox.utils.network as network  # noqa: E402

from coolbox.utils import (  # noqa: E402
    TOP_PORTS,
    AutoScanInfo,
...
def build_parser() -> ArgumentParser:
    """Create the argument parser used by the network scan CLI."""
    ...
    parser.add_argument(
        "--risk",
        action="store_true",
        help="Score risk for detected services",
    )
    parser.add_argument(
        "--ping",
        action="store_true",
        help="Ping hosts before scanning",
    )
    parser.add_argument(
        "--ping-timeout",
        type=float,
        default=1.0,
        help="Timeout for ping requests",
    )
    parser.add_argument(
        "--ping-concurrency",
        type=int,
...
async def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.json is not None:
        from coolbox.utils import auto_scan_results_to_dict
