...
from coolbox.utils.mouse_listener import get_global_listener, is_supported
from coolbox.utils.analysis.scoring_engine import ScoringEngine, tuning, softmax
from coolbox.utils.hover_tracker import HoverTracker
from ..base._fast_confidence import weighted_confidence as _weighted_confidence_np
from coolbox.utils import get_screen_refresh_rate
import logging
...
class ClickOverlay(tk.Toplevel):
    ...
    def _confirm_window(self) -> WindowInfo:
        ...
        x = int(self._click_x)
        y = int(self._click_y)
        stack = list_windows_at(x, y)
        info = WindowInfo(None)
        for win in stack:
            if win.pid in (self._own_pid, None):
                continue
            if is_transient_pid(win.pid):
                continue
            info = win
            break
        if info.pid is None and stack:
            info = stack[0]
